...
<template>
  <section class="border border-[var(--rr-line)] rounded-12px bg-[var(--rr-paper)] mx-auto max-w-980px p-34px max-md:p-24px shadow-[0_4px_24px_var(--rr-shadow)]">
    ...
    <div class="border-b border-[var(--rr-line)] py-24px">
      ...
      <div class="grid grid-cols-3 gap-10px max-md:grid-cols-1">
        <label
          v-for="mode in modes"
          :key="mode"
          class="border border-[var(--rr-line)] rounded-10px bg-[var(--rr-panel-muted)] flex min-w-0 items-center gap-10px p-14px cursor-pointer transition-[background-color,border-color,transform] duration-140 ease-out hover:border-[var(--rr-line-strong)] active:scale-[0.98]"
          :class="{ 'border-[var(--rr-green-line)] bg-[var(--rr-green-soft)] shadow-[0_2px_8px_var(--rr-shadow)]': form.mode === mode }"
        >
