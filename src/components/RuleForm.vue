...
<template>
  <section class="rr-card mx-auto max-w-980px p-34px max-md:p-24px">
    <header class="flex items-start justify-between gap-24px border-b border-[var(--rr-line)] pb-24px">
      ...
      <button type="button" class="rr-icon-btn h-36px w-36px flex-none" aria-label="关闭表单" @click="emit('cancel')">
        <span class="i-carbon:close" />
      </button>
    </header>

    <div class="border-b border-[var(--rr-line)] py-24px">
      <div class="mb-14px rr-section-title">
        基础信息
      </div>
    ...
    <div class="border-b border-[var(--rr-line)] py-24px">
      ...
      <div class="grid grid-cols-3 gap-10px max-md:grid-cols-1">
        <label
          v-for="mode in modes"
          :key="mode"
          class="rr-well flex min-w-0 items-center gap-10px p-14px cursor-pointer transition-[background-color,border-color] duration-140 ease-out hover:border-[var(--rr-line-strong)]"
