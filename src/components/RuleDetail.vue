  ...
  <section v-if="entry" class="border border-[var(--rr-line)] rounded-8px bg-[var(--rr-panel)] mx-auto max-w-980px p-34px max-md:p-24px">
    <header class="flex items-start justify-between gap-24px border-b border-[var(--rr-line)] pb-26px max-md:flex-col">
      <div class="min-w-0">
        ...
        <h1 class="m-0 mt-14px color-[var(--rr-ink)] text-28px font-700 leading-[1.18] tracking-0 [overflow-wrap:anywhere]">
          {{ entry.rule.name }}
        </h1>
      </div>
    ...
    <div v-if="!entry.isBuiltin" class="mt-18px flex items-center gap-10px border border-[var(--rr-orange-line)] rounded-8px bg-[var(--rr-orange-soft)] px-14px py-12px color-[var(--rr-orange-text)] text-13px">
